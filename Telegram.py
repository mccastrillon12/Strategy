...
with client:
    last_signal_data = None  # Guardar último contenido procesado
    last_vencida_data = None

    while True:
        try:
            signal = None
            for message in client.iter_messages(group_username, limit=10):
            ...
            # Crear una tupla identificadora de esta señal
            signal_key = (
                signal['symbol'], signal['tipo'],
                signal['entry'], signal['sl'], signal['tp3']
            )

            if signal_key == last_signal_data:
                time.sleep(10)
                continue

            last_signal_data = signal_key
            ...
            if minutos_pasados > 5:
                if signal_key == last_vencida_data:
                    mt5.shutdown()
                    time.sleep(10)
                    continue
